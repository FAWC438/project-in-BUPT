...
if __name__ == '__main__':
    ...
    while PC < RANGE:
        ...
        AR = PC
        logger.info('计数器地址存入地址寄存器')
        PC += 1
        logger.info('计数器加一')
        if PSW == '':
            PSW = '#'  # 默认状态
        DR = data[AR]
        logger.info('读取指令' + DR + '到DR')
        ...
        if ' ' in DR:
            ...
            temp = DR.split()
            AR = int(temp[1])
            logger.info('地址寄存器AR更新为：' + str(AR))
            IR = temp[0]
        else:
            # 不带目标地址的指令
            IR = DR
        logger.info('IR更新为：' + IR)
        ...
        if IR == 'CLA':
            ...
            if not DR.isdigit():
                ...
                time.sleep(2)
                break
            logger.info('从第' + str(AR) + '块地址中取出数据' + DR + '存入DR')
            AC += int(DR)
            if AC > 999999:
                # 溢出
                AC = AC % 999999
            ...
            if AR <= PC:
                exceptionFlag += 1
                logger.info('已触发死循环保护机制，重复循环10次时将退出，当前已循环' +
                            str(exceptionFlag)+'次')
            else:
                exceptionFlag = 0
            # 跳转
